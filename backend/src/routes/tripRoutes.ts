...
router.put('/:tripId', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
router.delete('/:tripId', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
// Trip Stops
router.post('/:tripId/stops', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
router.delete('/:tripId/stops/:stopId', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
router.post('/:tripId/stops/reorder', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
// Activities
router.post('/:tripId/stops/:stopId/activities', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const activity = await tripService.addActivityToStop(
      req.params.tripId,
      req.userId,
      req.params.stopId,
      { activityId, day, notes }
    );
...
// Budget
router.get('/:tripId/budget', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
