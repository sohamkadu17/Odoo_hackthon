...
// Packing Checklist
router.post('/:tripId/packing', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
...
router.get('/:tripId/packing', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
router.put('/:tripId/packing/:itemId', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const item = await tripUtilService.updatePackingItem(
      req.params.tripId,
      req.userId,
      req.params.itemId,
      req.body
...
router.delete('/:tripId/packing/:itemId', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
router.post('/:tripId/packing/reset', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
// Trip Notes
router.post('/:tripId/notes', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
router.get('/:tripId/notes', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
...
router.put('/:tripId/notes/:noteId', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
    ...
    const note = await tripUtilService.updateTripNote(
      req.params.tripId,
      req.userId,
      req.params.noteId,
      req.body
...
router.delete('/:tripId/notes/:noteId', async (req: AuthRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ message: 'Unauthorized' });
      return;
